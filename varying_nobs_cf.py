"""
CVCI-CF Simulation: Varying Observational Sample Size (N^obs)

Replicates CVCI paper Figure 14.4 (e) and (f) using Causal Forests:
  (e) θ^exp ≠ θ^obs, N^exp = 50, ε = 0.05, N^obs varies from 25 to 200
  (f) θ^exp ≠ θ^obs, N^exp = 1000, ε = 0.05, N^obs varies from 0 to 2000

Key features:
- Uses Causal Forests instead of DML for treatment effect estimation
...
Usage:
    python varying_nobs_cf.py                    # Run panel (e): N^exp=50
    python varying_nobs_cf.py --panel f          # Run panel (f): N^exp=1000
    python varying_nobs_cf.py --quick            # Quick test
    python varying_nobs_cf.py --mode plot --timestamp XXXXXX  # Plot saved results
"""
...
class FastCFModel(CFModel):
    """Faster CF with fewer trees and larger leaf size."""
...
def simulate_varying_nobs(n_exp=50, epsilon=0.05, d=5, sigma=1.0,
                          ...
                          use_fast=True, save_dir='simulation_results_nobs_cf'):
    """
    Simulate varying N^obs with fixed N^exp and ε using Causal Forests.
    ...
    Args:
        ...
        use_fast: Use fast CF (fewer trees)
    ...
    print(f"Running N^obs simulation with Causal Forests: n_exp={n_exp}, ε={epsilon}")
    ...
    for sim in range(n_sims):
        ...
        ate_exp = compute_exp_ate_cf(Z_exp, W_exp, Y_exp, method='difference')
        ...
        for nobs_idx, n_obs in enumerate(nobs_vals):
            ...
            if n_obs >= 20:  # Need minimum samples
                try:
                    if use_fast:
                        model_obs = FastCFModel(random_state=random_seed + sim)
                    else:
                        model_obs = CFModel(random_state=random_seed + sim)
                    ...
                    print(f"    Warning: Obs-only failed for n_obs={n_obs}: {e}")
                    ate_obs = np.nan
                    mse_obs = np.nan
            else:
                ate_obs = np.nan
            ...
            if n_obs >= 20:
                try:
                    ...
                    print(f"    Warning: CVCI-CF failed for n_obs={n_obs}: {e}")
                    ate_ours = ate_exp
                    mse_ours = mse_exp
                    lambda_opt = 0.0
            else:
...
def load_and_plot_nobs_results(timestamp, save_dir='simulation_results_nobs_cf', 
    ...
    ax.plot(df['n_obs'], df['mse_ours_mean'], 'orange', linestyle='-', marker='o',
            label=r'Ours (CF), $\beta(\hat{\theta}(\hat{\lambda}))$', linewidth=2, markersize=4)
    ...
    ax.set_title(rf"$\theta^{{exp}} \neq \theta^{{obs}}, N^{{exp}} = {n_exp}, \varepsilon = {epsilon}$ (Causal Forest)", 
                 fontsize=14)
    ...
    ax2.set_title(r'Optimal $\lambda$ vs $N^{obs}$ (Causal Forest)', fontsize=14)
    ax2.set_ylim([0, 1])
...
def run_panel_e():
    ...
    return simulate_varying_nobs(
        ...
        save_dir='simulation_results_nobs_cf_panel_e'
    )
...
def run_panel_f():
    ...
    return simulate_varying_nobs(
        ...
        save_dir='simulation_results_nobs_cf_panel_f'
    )


def plot_both_panels(timestamp_e, timestamp_f, output_file='nobs_both_panels_cf.png'):
    """
    Create side-by-side plot matching the paper's Figure 14.4 (e) and (f) using CF.
    """
    # Load both results
    with open(f'simulation_results_nobs_cf_panel_e/aggregated_nobs_{timestamp_e}.json', 'r') as f:
        data_e = json.load(f)
    with open(f'simulation_results_nobs_cf_panel_f/aggregated_nobs_{timestamp_f}.json', 'r') as f:
        data_f = json.load(f)
    ...
    ax1.set_title(rf"(e) $\theta^{{exp}} \neq \theta^{{obs}}, N^{{exp}} = {meta_e['n_exp']}, \varepsilon = {meta_e['epsilon']}$ (CF)", 
                 fontsize=12)
    ...
    ax2.grid(True, alpha=0.3)
    ax2.set_title(rf"(f) $\theta^{{exp}} \neq \theta^{{obs}}, N^{{exp}} = {meta_f['n_exp']}, \varepsilon = {meta_f['epsilon']}$ (CF)", 
                 fontsize=12)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run varying N^obs simulations with Causal Forests')
    ...
    parser.add_argument('--use-slow-cf', action='store_true',
                       help='Use slower CF with more trees')
    ...
    if args.mode == 'plot':
        if args.timestamp is None:
            ...
            sys.exit(1)
        save_dir = f'simulation_results_nobs_cf_panel_{args.panel}'
        ...
        if args.ultra_quick:
            ...
            print(f"FULL MODE (CF): {n_sims} sims, {lambda_bin} lambdas")
        
        use_fast = not args.use_slow_cf
        
        if args.panel == 'e':
            print(f"\nRunning Panel (e) with Causal Forest: N^exp=50, ε=0.05")
            save_dir = 'simulation_results_nobs_cf_panel_e'
            n_exp = 50
            if nobs_vals is None:
                nobs_vals = np.arange(25, 205, 5)
        else:
            print(f"\nRunning Panel (f) with Causal Forest: N^exp=1000, ε=0.05")
            save_dir = 'simulation_results_nobs_cf_panel_f'
            n_exp = 1000
            if nobs_vals is None:
        ...
        print("="*70)
        print(f"\nTo recreate plots:")
        print(f"  python varying_nobs_cf.py --mode plot --panel {args.panel} --timestamp {timestamp}")
